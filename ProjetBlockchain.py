...
def hashFunction(block):
    bloc = str(block.index) + str(block.timeStamp) + str(block.data) + str(block.previousHash) + str(block.signature)
...
class Blockchain(object):
    ...
    def expose(self):
        for block in self.chains:
            chain = "Block  "+str(block.index)+" {"+"\tindex: "+str(block.index)+"\t\ttimestamp: "+str(block.timeStamp)+ "\n\tprevious hash: "+str(block.previousHash) + "\t\tdata: "+str(block.data)+"\n\thash: "+str(block.hash)+"\t\t\tsignature: "+str(block.signature)+"\n}\t"
            print(str(chain))
